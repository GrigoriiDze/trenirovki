...
- [x] 4.4 **Прод проверен:** `/api/sync` на Vercel отвечает, тянет 26 упр. из Neon; `npm run shot -- <prod-url>` — хаб рендерит «сохранено» (значит задеплоенный клиент синкнулся)
- [ ] ⛔ 4.5 **Гейт:** Григорий вводит код на телефоне (`trenirovki-smoky.vercel.app`), ставит PWA — данные на месте

## Грабли деплоя (2026-09-03)

- `vercel.json` rewrite `(.*) → /index.html` съедал `/api/*`. Нужен негативный lookahead `(?!api/|...)`.
- **`"type":"module"` + Vercel + TS:** относительные импорты в `api/` обязаны быть с `.js` (`./_lib/db.js`), иначе ESM-Node в проде не резолвит скомпилированный код — `FUNCTION_INVOCATION_FAILED`. Локально tsx/vite прощают, прод — нет.
- Диагностика без Vercel CLI: временный `api/ping.ts` без импортов (проверить, что функции вообще поднимаются) + `try/catch` в хендлере, возвращающий `e.message` на 500.
